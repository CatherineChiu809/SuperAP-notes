...
public class BBoard {		// This is your main file that connects all classes.
	...
	public void loadUsers(String inputFile) throws FileNotFoundException {
		...
		while(sc.hasNextLine()){
			...
			int b = temp.indexOf(" ");
			if(b != -1){
				users.add(new User(temp.substring(0,b),temp.substring(b+1)));
			}
	...
	public void login(){
		...
		while(!isFound){
			...
			for(int i = 0; i<users.size(); i++){
				if(users.get(i).check(usrnm,pass)){
					currentUser = users.get(i);
					isFound = true;
					System.out.println();
					System.out.println("Welcome back "+ usrnm);
				}
			}
			if(!isFound){
	...
	private void display(){
		for(int it = 0; it<messages.size(); it++){
			if(!messages.get(it).isReply()){
				System.out.println("--------------------------------------------");
				messages.get(it).print(0);
				System.out.println("--------------------------------------------");
			}
		}
	}
	...
	*/
	...
	private void addTopic(){
		...
		messages.add(new Topic(author, subject, body, messageID));
	}
	...
	*/
	...
	while(true){
		...
		if(index >= 1 && index <= messages.size()){
			Message parent = messages.get(index - 1);
			System.out.print("Body: ");
			String newBody = sc.nextLine();

			String newSubj = "Re: " + parent.getSubject();
			int messageID = messages.size() + 1;
			Reply newReply = new Reply(currentUser.getUsername(), newSubj, newBody, messageID);

			parent.addChild(newReply);
			messages.add(newReply);
			break;
		} 
		else {
			System.out.println("Invalid message ID, try again");
		}
	}
}
	...
	private void setPassword(){
		Scanner sc = new Scanner(System.in);
		System.out.println("Enter your old password ('c' or 'C' for Menu)");
		...
		while(true){
			if(num!=0){
				System.out.println();
				System.out.print("Enter your old password ('c' or 'C' for Menu)");
				oldPass = sc.nextLine();
				if(oldPass.equals("c")||oldPass.equals("C")){
					return;
				}	
			}
			if(currentUser.setPassword(oldPass,"")){
